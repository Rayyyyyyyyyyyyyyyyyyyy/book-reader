...
```bash
tail -3 /private/tmp/claude-502/-Users-ray-shao-book-reader/c3ac6f98-aa36-4abe-b552-f2c05b2e18d6/tasks/ba9suc8zv.output   # 第三章 reflect
pgrep -fl "codex exec"                          # 還有沒有 codex 在跑
ls -t ~/.cache/book-v2-logs | head             # 各步 log 與 .last.md 回報
```

最後一行出現 `=== [05] pipeline done` 或 `=== [03] reflect done` 才算跑完。第五章已完成並 commit；第二章回頭看已重寫完成（8d07b01）；第三章回頭看 20:15 起跑。
...
| 第三章 | 擴寫定稿約 8,400 | 已校閱 | **跑中**（ba9suc8zv） |
...
**回頭看線**（只寫作者已校閱的章，追上正文就停）：第三章跑完 → 整節讀過、修掉抽象、重述故事或後設句 → commit → `part P1 "第一部｜我們怎麼走到一起，又失去彼此" "第一部｜我們怎麼走到一起，又失去彼此" "4,000–5,000"`（檔名標題可再議）→ `reflect 04` → 停，等作者校閱第五章。
