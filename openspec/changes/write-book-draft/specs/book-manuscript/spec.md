...
每章初稿 MUST 對照 `docs/chapter.md` 該章的規格完成：核心句到位、五件事 checklist（有來源或明確標示的場景、常見誤解、判斷界線、反例、行動問題）全數回答，篇幅依該文件目前的修訂原則與實際內容決定。

#### Scenario: 一章初稿完成時的驗收
- **WHEN** 任一章初稿寫完
- **THEN** 逐項核對 `docs/chapter.md` 該章的核心句、五件事與篇幅原則，缺項需補齊或在 tasks 註記偏離原因
...
### Requirement: 現場檔限額
現場檔片段 MUST 只出現在「讀者需要先看到現場，才會相信後面分析」的章，全書至多 3 處；第一章與第十二章現為明確標示的假設獨白，不計作已核對的原始現場檔。未有原始素材不得宣稱逐字現場紀錄。
